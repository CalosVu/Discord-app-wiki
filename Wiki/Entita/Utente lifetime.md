...
Tabella minimale, tre colonne: `id`, `discord_id` (`UNIQUE`), `username`. Nessuna FK verso `users`:
...
2. **La verifica di accesso lo promuove.** `VerificaAccessoService` controlla la tabella **per
   ...
   `scadenza = null`, senza nemmeno leggere `users` ([[Integrazione VuTracker]]).
...
- **Non esenta dai pagamenti.** Un lifetime che usa `!Donazione` percorre il flusso normale e paga
  come chiunque altro.
- **Non è collegato a `users`.** Un ID può stare qui senza avere una riga in `users`; in quel caso
